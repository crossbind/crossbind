...
const GENERATED_MARKER = 'Generated by @crossbind/plugin-react-native';
...
// An app that already adopts the lifecycle owns it: overwriting its scene delegate, or adding a
// second class of the same name to the build, would break a project that was already correct.
const adoptsScenes = (cfg) => Boolean(cfg.ios?.infoPlist?.UIApplicationSceneManifest);

function withSceneLifecycle(config) {
    if (adoptsScenes(config)) return config;
    ...
    const withSource = withDangerousMod(withManifest, ['ios', (cfg) => {
        ...
        // A scene delegate this plugin did not write belongs to the app; leave it in place.
        if (fs.existsSync(target) && !fs.readFileSync(target, 'utf8').includes(GENERATED_MARKER)) return cfg;
