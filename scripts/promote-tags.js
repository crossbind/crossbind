...
//     --primary ghcr.io/crossbind [--mirror registry.example/crossbind] [--dry-run]
//
...
//
// Promotion is idempotent: a missing tag is created, a tag already at the expected digest is
// accepted, and a tag at another digest fails. When a private mirror is explicitly supplied it is
// promoted first and canonical GHCR last, preserving GHCR as the commit point.
...
function resolve(ref) {
    ...
    try {
        return JSON.parse(r.stdout).digest ?? null;
    } catch {
        return null;
    }
}
...
function promote(registry, image, expected) {
    ...
    if (current === expected) {
        ok(`${stable} already resolves to ${expected.slice(0, 19)}… (idempotent)`);
        return;
    }
    if (current !== null) {
    ...
    if (DRY) {
        ok(`${stable} would be created at ${expected.slice(0, 19)}… (dry run)`);
        return;
    }

    const r = spawnSync(ORAS[0], [...ORAS.slice(1), 'tag', ...PLAIN, `${registry}/${image}@${expected}`, VERSION], { encoding: 'utf8' });
    if (r.status !== 0) {
        bad(`tagging ${stable}`, (r.stderr || '').trim().split('\n').slice(-2).join(' '));
        return;
    }
...
for (const image of IMAGES) {
    ...
    if (!d) {
        bad(`${PRIMARY}/${image}:${STAGING} does not exist - nothing to promote`);
        continue;
    }
    expected[image] = d;
...
if (failed) {
    console.log('\npromote-tags: aborted before touching any stable tag');
    process.exit(1);
}

if (MIRROR) {
    // A supplied mirror must already carry the same bytes under its staging tag.
    for (const image of IMAGES) {
    ...
    if (failed) {
        console.log('\npromote-tags: aborted - the mirror does not carry the staged bytes');
        process.exit(1);
    }
}

console.log('');
for (const registry of [...(MIRROR ? [MIRROR] : []), PRIMARY]) {
    for (const image of IMAGES) promote(registry, image, expected[image]);
}

// A release is finished only when every stable tag resolves to the staged digest everywhere the
// caller explicitly asked this command to promote it.
console.log('');
if (!DRY) {
    for (const registry of [PRIMARY, ...(MIRROR ? [MIRROR] : [])]) {
        for (const image of IMAGES) {
    ...
    if (!failed) ok(`every stable tag resolves to its staged digest on ${MIRROR ? 'both registries' : 'canonical GHCR'}`);
}
