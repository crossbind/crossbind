...
import { CREATE_COMMAND, SCAFFOLD_BUNDLERS, SCAFFOLD_FRAMEWORKS, SCAFFOLD_LINES, SCAFFOLD_TARGETS } from '../data.js';
    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '18px 20px', minHeight: 400, color: tokens.codeText, overflowX: 'auto' }}>
                {SCAFFOLD_LINES.slice(0, shown).map((line, index) => {
                    const prompt = line.kind === 'question' ? line.text.match(/^(\? [^›]+› )(.+)$/) : null;
                    return (
                        <div key={`${index}-${line.kind}`} style={{ color: toneOf(line.kind), whiteSpace: 'pre' }}>
                            {prompt ? (
                                <>
...
function ChoiceGroup({ tokens, label, children }) {
    return (
        <div>
            <div style={{ fontFamily: tokens.mono, fontSize: 12, letterSpacing: 1.2, color: tokens.textMuted, marginBottom: 8 }}>{label}</div>
...
export default function Scaffolder({ tokens }) {
    ...
    return (
        <section style={{ padding: '20px var(--content-x) 80px' }}>
            ...
            >
                <div
                    style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: 48, alignItems: 'start' }}
                >
                    <div>
                        <h2
                            style={{
                        ...
                        <p style={{ color: tokens.textDim, fontSize: 16, lineHeight: 1.6, marginBottom: 24, maxWidth: 500 }}>
                            The scaffolder asks five questions and prints a working repo — framework, bundler, target, and a sample C++ binding
                            already wired. <code style={code}>npm run dev</code> works on the spot.
                        </p>

                        <div style={{ fontFamily: tokens.mono, fontSize: 11, letterSpacing: 1.4, color: tokens.textMuted, margin: '22px 0 8px' }}>
                            RUN
                        </div>
                        <div style={{ marginBottom: 26 }}>
                            <CommandChip tokens={tokens} command={CREATE_COMMAND} />
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 24 }}>
                            <ChoiceGroup tokens={tokens} label="FRAMEWORKS">
                                {SCAFFOLD_FRAMEWORKS.map((f) => (
                                    <Chip key={f} tokens={tokens}>
                                        {f}
                                    </Chip>
                                ))}
                            </ChoiceGroup>
                            <ChoiceGroup tokens={tokens} label="BUNDLERS">
                                {SCAFFOLD_BUNDLERS.map((b) => (
                                    <Chip key={b} tokens={tokens}>
                                        {b}
                                    </Chip>
                                ))}
                            </ChoiceGroup>
